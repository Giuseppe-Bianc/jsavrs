...
fn test_function_call_argument_mismatch() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert_eq!(errors[0].message(), Some("Argument 2: expected i32, found string"));
}
...
fn test_return_type_mismatch() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("Return type mismatch"));
}
...
fn test_array_operations_valid() {
	let ast = vec![
		...
		Stmt::VarDeclaration {
			...
			type_annotation: Type::Array(Box::new(Type::I32), Box::new(Expr::null_expr(dummy_span()))),
			is_mutable: true,
...
fn test_break_outside_loop() {
	let ast = vec![Stmt::Break {
		span: dummy_span(),
	}];

	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("Break/continue outside loop"));
}
...
fn test_immutable_assignment() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("Assignment to immutable"));
}
...
fn test_binary_arithmetic_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("requires numeric operands"));
}
...
fn test_binary_comparison_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("requires compatible types"));
}
...
fn test_logical_operations_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert!(errors[0].message().unwrap().contains("requires boolean operands"));
}
...
fn test_bitwise_operations_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert_eq!(errors[0].message(), Some("Bitwise operator 'BitwiseOr' requires integer operands, found bool and i32"));
}
...
fn test_unary_negate_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert_eq!(errors[0].message(), Some("Negation requires numeric operand, found bool"));
}
...
fn test_unary_not_invalid() {
	...
	let errors = typecheck(ast);
	assert_eq!(errors.len(), 1);
	assert_eq!(errors[0].message(), Some("Logical not requires boolean operand, found i32"));
}