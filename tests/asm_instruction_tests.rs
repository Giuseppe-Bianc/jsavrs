...
fn test_arithmetic_instructions() {
    ...
    let add_instr = Instruction::Add {
        dest: Operand::reg64(GPRegister64::Rax),
        src: Operand::reg64(GPRegister64::Rbx)
    };
    ...
    let sub_instr = Instruction::Sub {
        dest: Operand::reg64(GPRegister64::Rcx),
    ...
    // Div instruction
    let div_instr = Instruction::Div {
        src: Operand::reg64(GPRegister64::R11)
    };
    assert_eq!(div_instr.mnemonic(), "div");
    assert_eq!(format!("{}", div_instr), "div r11");
    
    // Idiv instruction
    let idiv_instr = Instruction::Idiv {
        src: Operand::mem(GPRegister64::Rsp)
    };
    assert_eq!(idiv_instr.mnemonic(), "idiv");
    assert_eq!(format!("{}", idiv_instr), "idiv QWORD PTR [rsp]");
    
    // Inc instruction
    let inc_instr = Instruction::Inc {
        dest: Operand::reg64(GPRegister64::Rax)
    };
    ...
    // Dec instruction
    let dec_instr = Instruction::Dec {
        dest: Operand::reg32(GPRegister32::Ebx)
    };
    assert_eq!(dec_instr.mnemonic(), "dec");
    assert_eq!(format!("{}", dec_instr), "dec ebx");
    
    // Neg instruction
    let neg_instr = Instruction::Neg {
        dest: Operand::reg16(GPRegister16::Cx)
    };
    assert_eq!(neg_instr.mnemonic(), "neg");
    assert_eq!(format!("{}", neg_instr), "neg cx");
    
    // Adc instruction
    let adc_instr = Instruction::Adc {
        dest: Operand::reg8(GPRegister8::Al),
        src: Operand::imm8(5)
    };
    ...
    // Sbb instruction
    let sbb_instr = Instruction::Sbb {
        dest: Operand::reg64(GPRegister64::R12),
        src: Operand::reg64(GPRegister64::R13)
    };
...
fn test_logical_instructions() {
    ...
    let test_instr = Instruction::Test {
        op1: Operand::reg64(GPRegister64::Rax),
        op2: Operand::imm32(1)
    };
...
fn test_shift_rotate_instructions() {
    ...
    // Ror instruction
    let ror_instr = Instruction::Ror {
        dest: Operand::reg64(GPRegister64::Rdx),
        count: Operand::imm8(4)
    };
    assert_eq!(ror_instr.mnemonic(), "ror");
    assert_eq!(format!("{}", ror_instr), "ror rdx, 4");
    
    // Rcl instruction
    let rcl_instr = Instruction::Rcl {
        dest: Operand::reg32(GPRegister32::Esi),
        count: Operand::imm8(1)
    };
    ...
    // Rcr instruction
    let rcr_instr = Instruction::Rcr {
        dest: Operand::reg32(GPRegister32::Edi),
        count: Operand::reg8(GPRegister8::Cl)
    };
...
fn test_movement_instructions() {
    ...
    // Movsx instruction
    let movsx_instr = Instruction::Movsx {
        dest: Operand::reg64(GPRegister64::Rcx),
        src: Operand::reg32(GPRegister32::Edx)
    };
    assert_eq!(movsx_instr.mnemonic(), "movsx");
    assert_eq!(format!("{}", movsx_instr), "movsx rcx, edx");
    
    // Movsxd instruction
    let movsxd_instr = Instruction::Movsxd {
        dest: Operand::reg64(GPRegister64::Rsi),
        src: Operand::reg32(GPRegister32::Eax)
    };
    assert_eq!(movsxd_instr.mnemonic(), "movsxd");
    assert_eq!(format!("{}", movsxd_instr), "movsxd rsi, eax");
    
    // Movzx instruction
    let movzx_instr = Instruction::Movzx {
        dest: Operand::reg64(GPRegister64::Rdi),
        src: Operand::reg8(GPRegister8::Al)
    };
    assert_eq!(movzx_instr.mnemonic(), "movzx");
    assert_eq!(format!("{}", movzx_instr), "movzx rdi, al");
    
    // Lea instruction
    let lea_instr = Instruction::Lea {
        dest: Operand::reg64(GPRegister64::R8),
        src: Operand::mem(GPRegister64::Rsp)
    };
    ...
    // Push instruction
    let push_instr = Instruction::Push {
        src: Operand::reg64(GPRegister64::R9)
    };
    assert_eq!(push_instr.mnemonic(), "push");
    assert_eq!(format!("{}", push_instr), "push r9");
    
    // Pop instruction
    let pop_instr = Instruction::Pop {
        dest: Operand::reg64(GPRegister64::R10)
    };
    assert_eq!(pop_instr.mnemonic(), "pop");
    assert_eq!(format!("{}", pop_instr), "pop r10");
    
    // Xchg instruction
    let xchg_instr = Instruction::Xchg {
        op1: Operand::reg64(GPRegister64::R11),
        op2: Operand::reg64(GPRegister64::R12)
    };
    assert_eq!(xchg_instr.mnemonic(), "xchg");
    assert_eq!(format!("{}", xchg_instr), "xchg r11, r12");
    
    // Test different operand combinations
    let mov_reg_mem = Instruction::Mov {
        dest: Operand::reg32(GPRegister32::Eax),
        src: Operand::mem_disp(GPRegister64::Rbp, -8)
    };
    assert_eq!(format!("{}", mov_reg_mem), "mov eax, QWORD PTR [rbp - 8]");
    
    let mov_mem_reg = Instruction::Mov {
        dest: Operand::mem(GPRegister64::Rsp),
        src: Operand::reg64(GPRegister64::Rax)
    };
    assert_eq!(format!("{}", mov_mem_reg), "mov QWORD PTR [rsp], rax");
    
    let mov_reg_imm = Instruction::Mov {
        dest: Operand::reg16(GPRegister16::Ax),
...
fn test_comparison_instructions() {
    ...
    let cmp_instr = Instruction::Cmp {
        op1: Operand::reg64(GPRegister64::Rax),
        op2: Operand::reg64(GPRegister64::Rbx)
    };
    assert_eq!(cmp_instr.mnemonic(), "cmp");
    assert_eq!(format!("{}", cmp_instr), "cmp rax, rbx");
    
    // Test with different operand types
    let cmp_mem_imm = Instruction::Cmp {
        op1: Operand::mem(GPRegister64::Rsp),
        op2: Operand::imm32(42)
    };
    assert_eq!(format!("{}", cmp_mem_imm), "cmp QWORD PTR [rsp], 42");
    
    let cmp_reg_mem = Instruction::Cmp {
        op1: Operand::reg32(GPRegister32::Eax),
        op2: Operand::mem_disp(GPRegister64::Rbp, 16)
    };
    assert_eq!(format!("{}", cmp_reg_mem), "cmp eax, QWORD PTR [rbp + 16]");
    
    let cmp_reg_imm = Instruction::Cmp {
        op1: Operand::reg16(GPRegister16::Cx),
        op2: Operand::imm16(-100)
    };
    assert_eq!(format!("{}", cmp_reg_imm), "cmp cx, -100");
    
    let cmp_reg_label = Instruction::Cmp {
        op1: Operand::reg8(GPRegister8::Dl),
        op2: Operand::imm8(0xFF)
    };
    assert_eq!(format!("{}", cmp_reg_label), "cmp dl, -1");
}
...
fn test_jump_instructions() {
    ...
    let jmp_mem = Instruction::Jmp {
        target: Operand::mem(GPRegister64::Rax)
    };
    assert_eq!(format!("{}", jmp_mem), "jmp QWORD PTR [rax]");
    
    // Test with register operand as target
    let jmp_reg = Instruction::Jmp {
        target: Operand::reg64(GPRegister64::Rbx)
    };
...
fn test_sse_avx_instructions() {
    ...
    let movaps_instr = Instruction::Movaps {
        dest: Operand::xmm(XMMRegister::Xmm0),
        src: Operand::xmm(XMMRegister::Xmm1)
    };
    assert_eq!(movaps_instr.mnemonic(), "movaps");
    assert_eq!(format!("{}", movaps_instr), "movaps xmm0, xmm1");
    
    let movapd_instr = Instruction::Movapd {
        dest: Operand::xmm(XMMRegister::Xmm2),
        src: Operand::mem(GPRegister64::Rax)
    };
    assert_eq!(movapd_instr.mnemonic(), "movapd");
    assert_eq!(format!("{}", movapd_instr), "movapd xmm2, XMMWORD PTR [rax]");
    
    let movups_instr = Instruction::Movups {
        dest: Operand::mem(GPRegister64::Rbx),
        src: Operand::xmm(XMMRegister::Xmm3)
    };
    assert_eq!(movups_instr.mnemonic(), "movups");
    assert_eq!(format!("{}", movups_instr), "movups QWORD PTR [rbx], xmm3");
    
    let movupd_instr = Instruction::Movupd {
        dest: Operand::xmm(XMMRegister::Xmm4),
        src: Operand::xmm(XMMRegister::Xmm5)
    };
    assert_eq!(movupd_instr.mnemonic(), "movupd");
    assert_eq!(format!("{}", movupd_instr), "movupd xmm4, xmm5");
    
    let movss_instr = Instruction::Movss {
        dest: Operand::xmm(XMMRegister::Xmm6),
        src: Operand::xmm(XMMRegister::Xmm7)
    };
    assert_eq!(movss_instr.mnemonic(), "movss");
    assert_eq!(format!("{}", movss_instr), "movss xmm6, xmm7");
    
    let movsd_instr = Instruction::Movsd {
        dest: Operand::xmm(XMMRegister::Xmm8),
        src: Operand::mem(GPRegister64::Rcx)
    };
    assert_eq!(movsd_instr.mnemonic(), "movsd");
    assert_eq!(format!("{}", movsd_instr), "movsd xmm8, QWORD PTR [rcx]");
    
    let movdqa_instr = Instruction::Movdqa {
        dest: Operand::xmm(XMMRegister::Xmm9),
        src: Operand::xmm(XMMRegister::Xmm10)
    };
    assert_eq!(movdqa_instr.mnemonic(), "movdqa");
    assert_eq!(format!("{}", movdqa_instr), "movdqa xmm9, xmm10");
    
    let movdqu_instr = Instruction::Movdqu {
        dest: Operand::mem(GPRegister64::Rdx),
        src: Operand::xmm(XMMRegister::Xmm11)
    };
    ...
    let addps_instr = Instruction::Addps {
        dest: Operand::xmm(XMMRegister::Xmm0),
        src: Operand::xmm(XMMRegister::Xmm1)
    };
    ...
    let addpd_instr = Instruction::Addpd {
        dest: Operand::xmm(XMMRegister::Xmm2),
        src: Operand::mem(GPRegister64::Rax)
    };
    ...
    let andpd_instr = Instruction::Andpd {
        dest: Operand::xmm(XMMRegister::Xmm2),
        src: Operand::xmm(XMMRegister::Xmm3)
    };
    ...
    let orps_instr = Instruction::Orps {
        dest: Operand::xmm(XMMRegister::Xmm4),
        src: Operand::xmm(XMMRegister::Xmm5)
    };
    ...
    let orpd_instr = Instruction::Orpd {
        dest: Operand::xmm(XMMRegister::Xmm6),
        src: Operand::xmm(XMMRegister::Xmm7)
    };
    ...
    let xorps_instr = Instruction::Xorps {
        dest: Operand::xmm(XMMRegister::Xmm8),
        src: Operand::xmm(XMMRegister::Xmm9)
    };
    assert_eq!(xorps_instr.mnemonic(), "xorps");
    assert_eq!(format!("{}", xorps_instr), "xorps xmm8, xmm9");
    
    let xorpd_instr = Instruction::Xorpd {
        dest: Operand::xmm(XMMRegister::Xmm10),
        src: Operand::xmm(XMMRegister::Xmm11)
    };
    ...
    let cvtss2sd_instr = Instruction::Cvtss2sd {
        dest: Operand::xmm(XMMRegister::Xmm0),
        src: Operand::xmm(XMMRegister::Xmm1)
    };
...
fn test_fpu_instructions() {
    ...
    // FSTP instruction
    let fstp_mem = Instruction::Fstp {
        dest: Operand::mem(GPRegister64::Rcx)
    };
    assert_eq!(fstp_mem.mnemonic(), "fstp");
    assert_eq!(format!("{}", fstp_mem), "fstp QWORD PTR [rcx]");
    
    // FADD instruction
    let fadd_instr = Instruction::Fadd {
        src: Some(Operand::mem(GPRegister64::Rdx))
    };
    ...
    // FADDP instruction
    let faddp_instr = Instruction::Faddp {
        src: Some(Operand::reg64(GPRegister64::Rsi))
    };
    assert_eq!(faddp_instr.mnemonic(), "faddp");
    assert_eq!(format!("{}", faddp_instr), "faddp rsi");
    
    // FSUB instruction
    let fsub_instr = Instruction::Fsub {
        src: Some(Operand::mem(GPRegister64::Rdi))
    };
    assert_eq!(fsub_instr.mnemonic(), "fsub");
    assert_eq!(format!("{}", fsub_instr), "fsub QWORD PTR [rdi]");
    
    // FSUBP instruction
    let fsubp_instr = Instruction::Fsubp {
        src: Some(Operand::reg64(GPRegister64::R8))
    };
    assert_eq!(fsubp_instr.mnemonic(), "fsubp");
    assert_eq!(format!("{}", fsubp_instr), "fsubp r8");
    
    // FMUL instruction
    let fmul_instr = Instruction::Fmul {
        src: Some(Operand::mem(GPRegister64::R9))
    };
    assert_eq!(fmul_instr.mnemonic(), "fmul");
    assert_eq!(format!("{}", fmul_instr), "fmul QWORD PTR [r9]");
    
    // FMULP instruction
    let fmulp_instr = Instruction::Fmulp {
        src: Some(Operand::reg64(GPRegister64::R10))
    };
    assert_eq!(fmulp_instr.mnemonic(), "fmulp");
    assert_eq!(format!("{}", fmulp_instr), "fmulp r10");
    
    // FDIV instruction
    let fdiv_instr = Instruction::Fdiv {
        src: Some(Operand::mem(GPRegister64::R11))
    };
    assert_eq!(fdiv_instr.mnemonic(), "fdiv");
    assert_eq!(format!("{}", fdiv_instr), "fdiv QWORD PTR [r11]");
    
    // FDIVP instruction
    let fdivp_instr = Instruction::Fdivp {
        src: Some(Operand::reg64(GPRegister64::R12))
    };
...
fn test_bit_manipulation_instructions() {
    ...
    // BSR instruction
    let bsr_instr = Instruction::Bsr {
        dest: Operand::reg32(GPRegister32::Ecx),
        src: Operand::mem(GPRegister64::Rdx)
    };
    assert_eq!(bsr_instr.mnemonic(), "bsr");
    assert_eq!(format!("{}", bsr_instr), "bsr ecx, QWORD PTR [rdx]");
    
    // BT instruction
    let bt_instr = Instruction::Bt {
        dest: Operand::reg16(GPRegister16::Ax),
        src: Operand::imm8(5)
    };
    ...
    // BTC instruction
    let btc_instr = Instruction::Btc {
        dest: Operand::reg64(GPRegister64::Rsi),
        src: Operand::reg64(GPRegister64::Rdi)
    };
    assert_eq!(btc_instr.mnemonic(), "btc");
    assert_eq!(format!("{}", btc_instr), "btc rsi, rdi");
    
    // BTR instruction
    let btr_instr = Instruction::Btr {
        dest: Operand::reg32(GPRegister32::Edx),
        src: Operand::imm8(3)
    };
    ...
    // POPCNT instruction
    let popcnt_instr = Instruction::Popcnt {
        dest: Operand::reg64(GPRegister64::Rcx),
        src: Operand::reg64(GPRegister64::Rdx)
    };
    ...
    // LZCNT instruction
    let lzcnt_instr = Instruction::Lzcnt {
        dest: Operand::reg32(GPRegister32::Esi),
        src: Operand::mem(GPRegister64::Rbx)
    };
    ...
    // TZCNT instruction
    let tzcnt_instr = Instruction::Tzcnt {
        dest: Operand::reg16(GPRegister16::Dx),
        src: Operand::reg16(GPRegister16::Ax)
    };
    assert_eq!(tzcnt_instr.mnemonic(), "tzcnt");
    assert_eq!(format!("{}", tzcnt_instr), "tzcnt dx, ax");
    
    // Test with different combinations
    let bt_mem_imm = Instruction::Bt {
        dest: Operand::mem(GPRegister64::Rbp),
        src: Operand::imm16(16)
    };
    assert_eq!(format!("{}", bt_mem_imm), "bt QWORD PTR [rbp], 16");
    
    let popcnt_mem = Instruction::Popcnt {
        dest: Operand::reg64(GPRegister64::R8),
        src: Operand::mem(GPRegister64::R9)
    };
...
fn test_cmovcc_instructions() {
    // CMOVE instruction
    let cmove_instr = Instruction::Cmove {
        dest: Operand::reg64(GPRegister64::Rax),
        src: Operand::reg64(GPRegister64::Rbx)
    };
    ...
    // CMOVNE instruction
    let cmovne_instr = Instruction::Cmovne {
        dest: Operand::reg32(GPRegister32::Ecx),
        src: Operand::mem(GPRegister64::Rdx)
    };
    ...
    // CMOVG instruction
    let cmovg_instr = Instruction::Cmovg {
        dest: Operand::reg64(GPRegister64::Rsi),
        src: Operand::reg64(GPRegister64::Rdi)
    };
    ...
    // CMOVGE instruction
    let cmovge_instr = Instruction::Cmovge {
        dest: Operand::reg16(GPRegister16::Ax),
        src: Operand::reg16(GPRegister16::Bx)
    };
    assert_eq!(cmovge_instr.mnemonic(), "cmovge");
    assert_eq!(format!("{}", cmovge_instr), "cmovge ax, bx");
    
    // CMOVL instruction
    let cmovl_instr = Instruction::Cmovl {
        dest: Operand::reg32(GPRegister32::Edx),
        src: Operand::mem(GPRegister64::Rsp)
    };
    ...
    // CMOVLE instruction
    let cmovle_instr = Instruction::Cmovle {
        dest: Operand::reg64(GPRegister64::R8),
        src: Operand::reg64(GPRegister64::R9)
    };
    ...
    // CMOVA instruction
    let cmova_instr = Instruction::Cmova {
        dest: Operand::reg16(GPRegister16::Cx),
        src: Operand::reg16(GPRegister16::Dx)
    };
    assert_eq!(cmova_instr.mnemonic(), "cmova");
    assert_eq!(format!("{}", cmova_instr), "cmova cx, dx");
    
    // CMOVAE instruction
    let cmovae_instr = Instruction::Cmovae {
        dest: Operand::reg64(GPRegister64::R10),
        src: Operand::mem(GPRegister64::Rbp)
    };
    assert_eq!(cmovae_instr.mnemonic(), "cmovae");
    assert_eq!(format!("{}", cmovae_instr), "cmovae r10, QWORD PTR [rbp]");
    
    // CMOVB instruction
    let cmovb_instr = Instruction::Cmovb {
        dest: Operand::reg8(GPRegister8::Al),
        src: Operand::reg8(GPRegister8::Bl)
    };
    assert_eq!(cmovb_instr.mnemonic(), "cmovb");
    assert_eq!(format!("{}", cmovb_instr), "cmovb al, bl");
    
    // CMOVBE instruction
    let cmovbe_instr = Instruction::Cmovbe {
        dest: Operand::reg64(GPRegister64::R11),
        src: Operand::reg64(GPRegister64::R12)
    };
    ...
    let cmov_mem_reg = Instruction::Cmove {
        dest: Operand::mem(GPRegister64::Rax),
        src: Operand::reg64(GPRegister64::Rbx)
    };
    assert_eq!(format!("{}", cmov_mem_reg), "cmove QWORD PTR [rax], rbx");
    
    let cmov_reg_mem = Instruction::Cmovne {
        dest: Operand::reg32(GPRegister32::Esi),
        src: Operand::mem(GPRegister64::Rdi)
    };
