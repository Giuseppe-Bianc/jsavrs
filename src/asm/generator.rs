...
impl AsmGenerator {
    ...
    fn emit_formatted(&mut self, instr: &str, instr_arg: &str, allign: bool) {
        if allign {
            self.asm_code.push_str("    ");
        }
        self.asm_code
            .push_str(format!("{:<8} {}", instr, instr_arg).as_str());
        self.asm_code.push('\n');
    }
...
"#,
    ...
    fn generate_function(&mut self, func: &Function) -> Result<(), CompileError> {
        ...
        self.emit_formatted("push", "rbp", true);
        self.emit_formatted("mov", "rbp, rsp", true);
        ...
        if stack_size > 0 {
            self.emit_formatted(
                "sub",
                format!("rsp, {}", align_to(stack_size, 16)).as_str(),
                true,
            );
        }
        ...
        // Function epilogue
        self.emit(&format!(".return_{}:", func.name), false);
        self.emit_formatted("mov", "rsp, rbp", true);
        self.emit_formatted("pop", "rbp", true);
        self.emit("ret", true);
    ...
    fn generate_instruction(&mut self, inst: &Instruction) -> Result<(), CompileError> {
        match inst {
            ...
            _ => Err(self.error(&format!("Unsupported instruction: {:?}", inst))),
        }
    }
    ...
    fn generate_terminator(&mut self, term: &Terminator) -> Result<(), CompileError> {
        match term {
            Terminator::Return(value, ty) => {
                ...
                self.emit_formatted("jmp", ".return", true);
                Ok(())
            }
            Terminator::Branch(label) => {
                //self.emit(&format!("jmp .{}", label), true);
                self.emit_formatted("jmp", &format!(".{}", label), true);
                Ok(())
            }
            Terminator::ConditionalBranch {
                ...
                self.emit_formatted("cmp", &format!("{}, 0", cond_reg), true);
                self.emit_formatted("jne", &format!(".{}", true_label), true);
                self.emit_formatted("jmp", &format!(".{}", false_label), true);
                Ok(())
            }
    ...
    fn load_value(
        ...
        match &value.kind {
            ValueKind::Immediate(imm) => {
                ...
                self.emit_formatted("mov", &format!("{}, {}", reg, imm_str), true);
                Ok(reg.to_string())
            }
            _ => {
    ...
    fn get_value_offset(&self, value: &Value) -> Result<i32, CompileError> {
        match &value.kind {
            ValueKind::Local(name) | ValueKind::Temporary(name) => self
                .temp_offset_map
                .get(name)
                .copied()
    ...
    fn emit_load(
        ...
        match ty {
            IrType::F32 => {
                self.emit_formatted("movss", &format!("{}, [{} + {}]", dest, base, offset), true);
                Ok(())
            }
            IrType::F64 => {
                self.emit_formatted("movsd", &format!("{}, [{} + {}]", dest, base, offset), true);
                Ok(())
            }
            _ => {
    ...
    fn emit_store(
        ...
        match ty {
            IrType::F32 => {
                self.emit_formatted("movss", &format!("[{} + {}], {}", base, offset, src), true);
                Ok(())
            }
            IrType::F64 => {
                self.emit_formatted("movsd", &format!("[{} + {}], {}", base, offset, src), true);
                Ok(())
            }
            _ => {
                let size = self.type_size(ty);
    ...
    fn generate_binary_op(
        ...
        self.emit_formatted(op_asm, &format!("{}, {}", left_reg, right_reg), true);

        if matches!(op, IrBinaryOp::Equal) {
            self.emit_formatted("sete", "al", true);
            self.emit_formatted("movzx", "rax, al", true);
        }
    ...
    fn generate_call(
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            if self.is_float(&arg.ty) {
                self.emit_formatted("movss", &format!("{}, {}", reg, value_reg), true);
            } else {
                self.emit_formatted("mov", &format!("{}, {}", reg, value_reg), true);
            }
        }

        // Call function
        self.emit_formatted("call", func, true);
