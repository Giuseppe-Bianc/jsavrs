...
impl RBasicBlock {
    ...
    pub fn terminator(&self) -> &RTerminator { &self.terminator }
    pub fn terminator_mut(&mut self) -> &mut RTerminator { &mut self.terminator }
    pub fn set_terminator(&mut self, t: RTerminator) { self.terminator = t; }

    pub fn scope(&self) -> Option<RScopeId> { self.scope } // if RScopeId: Copy
    // alternatively (safe regardless of Copy):
    // pub fn scope(&self) -> Option<&RScopeId> { self.scope.as_ref() }
    pub fn set_scope(&mut self, s: RScopeId) { self.scope = Some(s); }
    pub fn clear_scope(&mut self) { self.scope = None; }
}
