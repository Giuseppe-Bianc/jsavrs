...
impl DominanceInfo {
    ...
    pub fn compute_dominators(&mut self, cfg: &ControlFlowGraph) -> Result<(), String> {
        let entry_idx = cfg.get_entry_block_index()
            .ok_or_else(|| "CFG has no entry block".to_string())?;
        ...
        while changed {
            ...
            for &node in &post_order {
                ...
                let preds: Vec<NodeIndex> = cfg.graph()
                ...
                for &pred in &preds {
                    if pred != new_idom {
                        if let Some(Some(pred_idom)) = self.idom.get(&pred) {
    ...
    fn intersect(&self, mut node1: NodeIndex, mut node2: NodeIndex, idom: &HashMap<NodeIndex, Option<NodeIndex>>) -> NodeIndex {
        while node1 != node2 {
