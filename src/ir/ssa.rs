...
impl SsaTransformer {
    ...
    pub fn transform_function(&mut self, func: &mut Function) -> Result<(), String> {
        ...
        self.verify_ssa_form(func)
            .map_err(|e| format!("SSA verification failed: {}", e))?;
    ...
    fn process_block(&mut self, func: &mut Function, block_idx: NodeIndex) {
        ...
        for (i, ty, var_name) in phi_updates {
            ...
            let source_span = block.instructions[i].result.as_ref().unwrap().debug_info.as_ref().unwrap().source_span.clone();
            let new_value = Value::new_temporary(self.temp_counter, ty)
                .with_debug_info(Some(var_name.clone().into()), source_span);
            self.temp_counter += 1;
            ...
            match &mut instruction.kind {
                InstructionKind::Store { value, dest } => {
                    ...
                    if let ValueKind::Temporary(temp_id) = &dest.kind {
                        ...
                        let new_value = Value::new_temporary(self.temp_counter, ty)
                            .with_debug_info(Some(var_name.clone().into()), dest.debug_info.as_ref().map(|d| d.source_span.clone()).unwrap_or_default());
                        self.temp_counter += 1;
    ...
    /// Checks that each variable is assigned exactly once along all execution paths.
    pub fn verify_ssa_form(&self, func: &Function) -> Result<(), String> {
        let mut assignments = HashMap::new();
        
        // Collect all assignments in the function
            ...
            for instruction in &block.instructions {
                if let Some(result) = &instruction.result {
                    if let ValueKind::Temporary(temp_id) = &result.kind {
                        ...
                        let count = assignments.entry(var_name.clone()).or_insert(0);
                        *count += 1;
                        
                        // Check if this variable already has an assignment
                        if *count > 1 {
                            return Err(format!("Variable '{}' assigned more than once", var_name));
                        }
                    }
                }
            }
    ...
    pub fn verify_semantic_preservation(&self, original_func: &Function, transformed_func: &Function) -> Result<(), String> {
